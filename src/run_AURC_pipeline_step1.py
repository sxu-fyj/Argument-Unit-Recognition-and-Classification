...
from models2 import Token_pipeline_gru_crf_step#0
#from models2 import Token_pipeline_idcnn_crf_step#1
#from models2 import Token_pipeline_lstm_crf_step#Li,2
...
sys.stdout = Logger("./0_pipeline_c_step1.txt")
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        for input_ids, IO_label_ids, label_ids, IO_tag in zip(batch_input_ids, batch_IO_label_ids, batch_label_ids, IO_tags):
            ...
            correct_tags = [l for t, l in zip(input_tokens[1:seq_len], IO_tag[1:seq_len]) if not t.startswith('##')]
    ...
    # flatten
    YT, YP = [], []
    for t, p in zip(y_IO_true, y_pred):
        YT += t
        YP += p
...
def main():
    ...
    # 数据位置
    parser.add_argument('--output_dir', type=str, default='../models_gpu_c_pip_0/',
    ...
    if args.train:
        ...
        # Model5
        model = Token_pipeline_gru_crf_step(
