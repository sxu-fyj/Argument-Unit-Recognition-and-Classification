...
from utils import InputFeatures_span, get_data_with_labels

from models import TokenBERT_span2
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_label_ids, batch_start_ids, batch_end_ids, batch_argument_ids = batch
        # print(batch_input_ids.shape[0])
        # forward pass
        start_loss, end_loss = model(
            input_ids=batch_input_ids,
            token_type_ids=batch_sentence_ids,
            attention_mask=batch_input_mask,
            labels=batch_label_ids,
            start_positions=batch_start_ids,
            end_positions=batch_end_ids,
            # batch_size = batch_input_ids.shape[0]

        )
        # for j, example_index in enumerate(example_indices):
        #     loss_inference = criterion(logits, batch_argument_ids)
        #
        ...
        #loss_inference = criterion(logits, batch_argument_ids)
        # label = batch_argument_ids.cpu().numpy().tolist()[0]
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        batch_correct_tags = []
        for input_ids, label_ids, argument_ids, start_ids, start_tag, end_ids, end_tag in zip(batch_input_ids,
                                                                                                    batch_label_ids,
                                                                                                    batch_argument_ids,
                                                                                                    batch_start_ids,
                                                                                                    start_tags,
            ...
            argument_ids = argument_ids.cpu().tolist()
            label_ids = label_ids.cpu().tolist()
            start_ids = start_ids.cpu().tolist()
            end_ids = end_ids.cpu().tolist()
            #if type(tags) != list:
            start_tag = start_tag.cpu().tolist()
            end_tag = end_tag.cpu().tolist()
            ...
            input_tokens = tokenizer.convert_ids_to_tokens(input_ids)
                ...
                #                      not t.startswith('##')]
                # tags = [0] * len(input_tokens)
                # tags[0] = 1
                # # tags需要一个全０的ｔｅｎｓｏｒ，１００维
...
def main():
    ...
    for topic, AD in AURC_DATA_dict.items():
        for ad in AD:
            ...
            sequence_dict['argumnet_ids'] = ad['is_argument']
            # sequence_dict['start_ids'] = ad['start'] + 1
            # sequence_dict['end_ids'] = ad['end'] + 1
            ...
            FE = [
                InputFeatures_span(
                    ...
                    argumnet_ids=sequence_dict['argumnet_ids'],
    ...
    all_argumnet_ids = torch.tensor([f.argumnet_ids for f in train_features], dtype=torch.long)
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_start_ids,
                               all_end_ids, all_argumnet_ids)
    ...
    all_argumnet_ids = torch.tensor([f.argumnet_ids for f in eval_features], dtype=torch.long)
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_start_ids, all_end_ids,
                              all_argumnet_ids)
    ...
    all_argumnet_ids = torch.tensor([f.argumnet_ids for f in test_features], dtype=torch.long)
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_start_ids, all_end_ids,
                              all_argumnet_ids)
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            print("TRAIN:  Pre. %.3f | Rec. %.3f | F1 %.4f" % (p_train, r_train, f1s_train))

            # EVALUATION: DEV SET
            y_true_dev, y_pred_dev, p_dev, r_dev, f1s_dev = evaluation(
                eval_dataloader, model=model, device=device, tokenizer=tokenizer)
            print("EVAL:   Pre. %.3f | Rec. %.3f | F1 %.4f | BEST F1: %.3f" % (p_dev, r_dev, f1s_dev, best_f1s_dev),
                  best_epoch)
            ...
            if f1s_dev > best_f1s_dev:
                ...
                print("TEST:   Pre. %.3f | Rec. %.3f | F1 %.4f" % (p_test, r_test, f1s_test))
                if args.save_model:
                    # Save Model
    ...
    if args.eval:
        ...
        y_true_dev, y_pred_dev, p_dev, r_dev, f1s_dev = evaluation(
            eval_dataloader, model=model, device=device, tokenizer=tokenizer)
        print("EVAL:   Pre. %.3f | Rec. %.3f | F1 %.4f" % (p_dev, r_dev, f1s_dev))
