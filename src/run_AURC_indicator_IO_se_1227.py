...
from models2 import Token_indicator_multitask_se_1227
import math
import os
...
sys.stdout = Logger("./0_1227repeat.txt")


def _print_args(self):
    n_trainable_params, n_nontrainable_params = 0, 0
    for p in self.model.parameters():
        n_params = torch.prod(torch.tensor(p.shape)).item()  # prod返回所有元素的乘积，维度，乘积
        if p.requires_grad:
...
def reset_params(model, device):
    # print(self.model.state_dict())
    # for n,p in self.model.named_parameters():
    #     if p.requires_grad:
    #         print(n)
    # config = BertConfig.from_pretrained('bert-base-uncased/bert-base-uncased-config.json')

    # self.model = opt.model_class(opt, config).to(opt.device)#模型部分
    bert_config = BertConfig.from_pretrained('../model_base/config.json')

    bert = BertModel.from_pretrained('../model_base/pytorch_model.bin', config=bert_config).to(device)
    #n,p = model.named_parameters()
    for n, p in model.named_parameters():
        # if p.requires_grad:
        if 'bert' in n:
            for bert_n, bert_p in bert.named_parameters():
                if n.replace('bert.', '') == bert_n:
                    p.data = bert_p.data
                    # print(n)
        else:
            if p.requires_grad:
                ...
                if len(p.shape) > 1:
                    ...
                    torch.nn.init.uniform_(p, a=-stdv, b=stdv)  # 均匀分布


def training(i, args, tokenizer, all_input_tokens_dev, all_input_tokens_test, config, train_dataloader, eval_dataloader, test_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    best_p_dev, best_r_dev, best_f1s_dev = 0.0, 0.0, 0.0
    best_epoch = None
    task = '_'.join(['aurc', args.target_domain[:2].lower()])
    a = args.output_dir + 'f1_checkpoint/repeat_' + str(i) + '/'
    print(not os.path.exists(a))
    if not os.path.exists(a):
        os.mkdir(a)
    MODEL_PATH = os.path.join(a, '{}_token.pt'.format(task+str(1)))
    print(MODEL_PATH)
    CONFIG_PATH = os.path.join(a, '{}_config.json'.format(task+str(1)))
    print(CONFIG_PATH)
    PREDICTIONS_DEV = os.path.join(a, '{}_predictions_dev.json'.format(task+str(1)))
    print(PREDICTIONS_DEV)
    PREDICTIONS_TEST = os.path.join(a, '{}_predictions_test.json'.format(task+str(1)))
    print(PREDICTIONS_TEST)



    p_test, r_test, f1s_test = 0, 0, 0
            ...
            if step % 50 == 0:
                ...
                if f1s_dev >= best_f1s_dev:
                    ...
                    if args.save_prediction:
                        ...
                        DATA_DEV = get_data_with_labels(all_input_tokens_dev, y_true_dev, y_pred_dev, y2_pred_dev,
                                                        stance1_dev, stance2_dev)
                        with open(PREDICTIONS_DEV, 'w') as f:
                            json.dump(DATA_DEV, f, sort_keys=True, indent=4, separators=(',', ': '))
                        # TEST
                        DATA_TEST = get_data_with_labels(all_input_tokens_test, y_true_test, y_pred_test, y2_pred_test,
                                                         stance1_test, stance2_test)
    ...
    if args.eval:
        # Model
        model = Token_indicator_multitask_se_1227(
...
def main():
    ...
    # 数据位置
    parser.add_argument('--output_dir', type=str, default='../models_gpu0/',
    ...
    if args.train:
        ...
        if not os.path.exists(args.output_dir + 'log/'):
            os.mkdir(args.output_dir + 'log/')
        if not os.path.exists(args.output_dir + 'f1_checkpoint/'):
            os.mkdir(args.output_dir + 'f1_checkpoint/')
        f_out = open('log/'+ 'Token_indicator_multitask_se_1227' +'_'+ args.target_domain +'_val.txt', 'w', encoding='utf-8')
        ...
        for i in range(10):
            print('repeat: ', (i))
            f_out.write('repeat: '+str(i))
            reset_params(model, device)

            max_test_pre, max_test_rec, max_test_f1= training(i, args, tokenizer, all_input_tokens_dev, all_input_tokens_test, config, train_dataloader, eval_dataloader, test_dataloader, model=model, device=device,
                                                              optimizer=optimizer, scheduler=scheduler, max_grad_norm=args.max_grad_norm)
            print('max_test_pre: {:.4f}, max_test_rec: {:.4f}, max_test_f1: {:.4f}'.format(max_test_pre, max_test_rec, max_test_f1))
            f_out.write('max_test_pre: {:.4f}, max_test_rec: {:.4f}, max_test_f1: {:.4f} \n'.format(max_test_pre, max_test_rec, max_test_f1))
            print('#' * 100)
    ...
    # if args.eval:
    #     # Model
    #     model = Token_indicator_multitask_se_1223_2(
    #         model_name='../model_base/',
    #         #            model_name=args.pretrained_weights,
    #         num_labels=args.num_labels,
    #         output_hidden_states=False,
