...
from utils import InputFeatures_IO, get_data_with_labels3

#from models2 import Token_0303
#from models2 import Token_0303_LSTM_CRF
#from models2 import Token_0304_IDCNN_CRF
from models2 import Token_0302
...
sys.stdout = Logger("./3_0325_Li.txt")
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_argument_input_ids, batch_argument_input_mask, batch_argument_sentence_ids, batch_label_ids, batch_IO_label_ids = batch
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        batch_bieo_stance_tags = tagssss
...
def main():
    ...
    all_argument_input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long)
    all_argument_input_mask = torch.tensor([f.attention_mask for f in train_features], dtype=torch.long)
    all_argument_segment_ids = torch.tensor([f.token_type_ids for f in train_features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in train_features], dtype=torch.long)
    all_IO_ids = torch.tensor([f.IO_label_ids for f in train_features], dtype=torch.long)
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_IO_ids)
    ...
    all_argument_input_ids = torch.tensor([f.input_ids for f in eval_features], dtype=torch.long)
    all_argument_input_mask = torch.tensor([f.attention_mask for f in eval_features], dtype=torch.long)
    all_argument_segment_ids = torch.tensor([f.token_type_ids for f in eval_features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in eval_features], dtype=torch.long)
    all_IO_ids = torch.tensor([f.IO_label_ids for f in eval_features], dtype=torch.long)
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_IO_ids)
    ...
    all_argument_input_ids = torch.tensor([f.input_ids for f in test_features], dtype=torch.long)
    all_argument_input_mask = torch.tensor([f.attention_mask for f in test_features], dtype=torch.long)
    all_argument_segment_ids = torch.tensor([f.token_type_ids for f in test_features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in test_features], dtype=torch.long)
    all_IO_ids = torch.tensor([f.IO_label_ids for f in test_features], dtype=torch.long)
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_IO_ids)
        ...
        for epoch in range(args.epochs):
            ...
            if f1s_dev > best_f1s_dev:
                ...
                if args.save_prediction:
                    ...
                    DATA_DEV = get_data_with_labels3(all_input_tokens_dev, y_true_dev, y_pred_dev)
                    with open(PREDICTIONS_DEV, 'w') as f:
                        json.dump(DATA_DEV, f, sort_keys=True, indent=4, separators=(',', ': '))
                    # TEST
                    DATA_TEST = get_data_with_labels3(all_input_tokens_test, y_true_test, y_pred_test)
