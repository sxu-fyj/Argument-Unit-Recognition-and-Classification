...
from utils import InputFeatures_pipeline_IO_pointer, get_data_with_labels2

#from models2 import Token_pipeline_0211
from models2 import Token_pipeline_0306_pointer
...
sys.stdout = Logger("./3_0304_pipeline_IDCNN.txt")
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_argument_input_ids, batch_argument_input_mask, batch_argument_sentence_ids, batch_span_label, batch_label_ids, batch_IO_label_ids, batch_stance_label_ids, batch_argument_label_ids, batch_topic_indices = batch

        #id2IO = {0: 'O', 1: 'CON', 2: 'PRO'}
        #IO2ids = {'O': 0, 'CON': 1, 'PRO': 2}

        with torch.no_grad():
            start_label, end_label = model(
                md='IO_inference',
            ...
            for s_num, e_num in zip(start_label, end_label):
                pre_tag = [0] * len(batch_label_ids.cpu().tolist()[0])
...
def main():
    ...
    for topic, AD in AURC_DATA_dict.items():
        for ad in AD:
            ...
            #for ii in input_IO_labels:
            starts = []
            ends = []
            before = 0
            for j in range(len(input_IO_labels)):
                if input_IO_labels[j] != before and input_IO_labels[j] != 0:
                    starts.append(j)
                if j != len(input_IO_labels) - 1:
                    if input_IO_labels[j] != input_IO_labels[j + 1] and input_IO_labels[j] != 0:
                        ends.append(j)
                else:
                    if input_IO_labels[j] == input_IO_labels[j - 1] and input_IO_labels[j] != 0:
                        ends.append(j)
                before = input_IO_labels[j]
            span_label=[[0,0],[0,0]]
            for i in range(min(len(starts),2)):
                span_label[i][0]=starts[i]
                span_label[i][1]=ends[i]+1

            FE = [
                InputFeatures_pipeline_IO_pointer(
    ...
    all_span_label = torch.tensor([f.span_labels for f in train_features], dtype=torch.long)
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_span_label, all_label_ids, all_IO_label_ids, all_stance_label_ids, all_argument_label_ids,all_topics)
    ...
    all_span_label = torch.tensor([f.span_labels for f in eval_features], dtype=torch.long)
    ...
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_span_label, all_label_ids, all_IO_label_ids, all_stance_label_ids, all_argument_label_ids,all_topics)
    ...
    all_span_label = torch.tensor([f.span_labels for f in test_features], dtype=torch.long)
    ...
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_span_label, all_label_ids, all_IO_label_ids, all_stance_label_ids, all_argument_label_ids,all_topics)
    ...
    if args.train:
        ...
        # Model
        model = Token_pipeline_0306_pointer(
