...
from utils import InputFeatures_stance, get_data_with_labels

from models import TokenBERT_stance
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, s, batch_topic_input_ids, batch_topic_input_mask, batch_topic_sentence_ids, t, batch_label_ids, batch_stance_ids = batch
        ...
        loss = model(
            ...
            sentence_mask = s,
            topic_input_ids= batch_topic_input_ids,
            topic_token_type_ids=batch_topic_sentence_ids,
            topic_attention_mask=batch_topic_input_mask,
            topic_mask=t,
            labels=batch_label_ids,
            stance_label= batch_stance_ids,
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        y_true += batch_stance_ids.cpu().tolist()
        y_pred += batch_stance_tags.cpu().tolist()
...
def main():
    ...
    for topic, AD in AURC_DATA_dict.items():
        for ad in AD:
            ...
            s=sequence_dict['input_ids'].index(101)
            e=sequence_dict['input_ids'].index(102)
            sentence_mask = sequence_dict['attention_mask'].copy()
            sentence_mask[s] = 0
            sentence_mask[e] = 0
            topic_dict = tokenizer.encode_plus(topic, max_length=10, pad_to_max_length=True, add_special_tokens=True)
            topic_s = topic_dict['input_ids'].index(101)
            topic_e = topic_dict['input_ids'].index(102)
            topic_mask = topic_dict['attention_mask'].copy()
            topic_mask[topic_s] = 0
            topic_mask[topic_e] = 0
            ...
            FE = [
                InputFeatures_stance(
                    input_ids=sequence_dict['input_ids'],
                    attention_mask=sequence_dict['attention_mask'],
                    token_type_ids=sequence_dict['token_type_ids'],
                    s=sentence_mask,
                    topic_input_ids=topic_dict['input_ids'],
                    topic_attention_mask=topic_dict['attention_mask'],
                    topic_token_type_ids=topic_dict['token_type_ids'],
                    t=topic_mask,
                    label_ids=sequence_dict['label_ids'],
                    stance_ids=sequence_dict['stance_ids'],
    ...
    all_s = torch.tensor([f.s for f in train_features], dtype=torch.long)

    all_topic_input_ids = torch.tensor([f.topic_input_ids for f in train_features], dtype=torch.long)
    all_topic_input_mask = torch.tensor([f.topic_attention_mask for f in train_features], dtype=torch.long)
    all_topic_segment_ids = torch.tensor([f.topic_token_type_ids for f in train_features], dtype=torch.long)
    all_t = torch.tensor([f.t for f in train_features], dtype=torch.long)

    all_label_ids = torch.tensor([f.label_ids for f in train_features], dtype=torch.long)
    all_stance_ids = torch.tensor([f.stance_ids for f in train_features], dtype=torch.long)
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_s, all_topic_input_ids, all_topic_input_mask, all_topic_segment_ids, all_t, all_label_ids, all_stance_ids)
    ...
    all_s = torch.tensor([f.s for f in eval_features], dtype=torch.long)

    all_topic_input_ids = torch.tensor([f.topic_input_ids for f in eval_features], dtype=torch.long)
    all_topic_input_mask = torch.tensor([f.topic_attention_mask for f in eval_features], dtype=torch.long)
    all_topic_segment_ids = torch.tensor([f.topic_token_type_ids for f in eval_features], dtype=torch.long)
    all_t = torch.tensor([f.t for f in eval_features], dtype=torch.long)

    all_label_ids = torch.tensor([f.label_ids for f in eval_features], dtype=torch.long)
    all_stance_ids = torch.tensor([f.stance_ids for f in eval_features], dtype=torch.long)
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_s, all_topic_input_ids, all_topic_input_mask, all_topic_segment_ids, all_t, all_label_ids, all_stance_ids)
    ...
    all_s = torch.tensor([f.s for f in test_features], dtype=torch.long)

    all_topic_input_ids = torch.tensor([f.topic_input_ids for f in test_features], dtype=torch.long)
    all_topic_input_mask = torch.tensor([f.topic_attention_mask for f in test_features], dtype=torch.long)
    all_topic_segment_ids = torch.tensor([f.topic_token_type_ids for f in test_features], dtype=torch.long)
    all_t = torch.tensor([f.t for f in test_features], dtype=torch.long)

    all_label_ids = torch.tensor([f.label_ids for f in test_features], dtype=torch.long)
    all_stance_ids = torch.tensor([f.stance_ids for f in test_features], dtype=torch.long)
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_s, all_topic_input_ids, all_topic_input_mask, all_topic_segment_ids, all_t, all_label_ids, all_stance_ids)
