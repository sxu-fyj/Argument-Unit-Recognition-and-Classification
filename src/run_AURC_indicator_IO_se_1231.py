...
from models2 import Token_indicator_multitask_se_1231
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        label_tags3 = tagssss[3]
        label_tags4 = tagssss[4]
        label_tags5 = tagssss[5]
        stance_tags1 = tagssss[6]
        stance_tags2 = tagssss[7]
        ...
        batch_correct_tags2 = []
        batch_correct_label_ids3 = []
        batch_correct_tags3 = []
        batch_correct_label_ids4 = []
        batch_correct_tags4 = []
        batch_correct_label_ids5 = []
        batch_correct_tags5 = []


        for input_ids, IO_label_ids, label_ids, IO_tag, tags1, tags2, tags3, tags4, tags5 in zip(batch_input_ids, batch_IO_label_ids, batch_label_ids, IO_tags, label_tags1, label_tags2, label_tags3, label_tags4, label_tags5):
            ...
            #3
            correct_label_ids = [l for t, l in zip(input_tokens[1:seq_len], label_ids[1:seq_len]) if not t.startswith('##')]
            correct_tags3 = [l for t, l in zip(input_tokens[1:seq_len], tags3[1:seq_len]) if not t.startswith('##')]
            seq = " ".join(input_tokens[1:seq_len]).replace(' ##', '')
            assert len(correct_label_ids) == len(seq.split(' '))
            batch_correct_label_ids3.append(correct_label_ids)  # 真实值
            batch_correct_tags3.append(correct_tags3)  # 预测值
            # 4
            correct_label_ids = [l for t, l in zip(input_tokens[1:seq_len], label_ids[1:seq_len]) if not t.startswith('##')]
            correct_tags4 = [l for t, l in zip(input_tokens[1:seq_len], tags4[1:seq_len]) if not t.startswith('##')]
            seq = " ".join(input_tokens[1:seq_len]).replace(' ##', '')
            assert len(correct_label_ids) == len(seq.split(' '))
            batch_correct_label_ids4.append(correct_label_ids)  # 真实值
            batch_correct_tags4.append(correct_tags4)  # 预测值
            # 5
            correct_label_ids = [l for t, l in zip(input_tokens[1:seq_len], label_ids[1:seq_len]) if not t.startswith('##')]
            correct_tags5 = [l for t, l in zip(input_tokens[1:seq_len], tags5[1:seq_len]) if not t.startswith('##')]
            seq = " ".join(input_tokens[1:seq_len]).replace(' ##', '')
            assert len(correct_label_ids) == len(seq.split(' '))
            batch_correct_label_ids5.append(correct_label_ids)  # 真实值
            batch_correct_tags5.append(correct_tags5)  # 预测值
        ...
        y3_true += batch_correct_label_ids3
        y3_pred += batch_correct_tags3

        y4_true += batch_correct_label_ids4
        y4_pred += batch_correct_tags4

        y5_true += batch_correct_label_ids5
        y5_pred += batch_correct_tags5
    ...
    YT3, YP3 = [], []
    for t, p in zip(y3_true, y3_pred):
        YT3 += t
        YP3 += p
    assert len(YT3) == len(YP3)

    YT4, YP4 = [], []
    for t, p in zip(y4_true, y4_pred):
        YT4 += t
        YP4 += p
    assert len(YT4) == len(YP4)

    YT5, YP5 = [], []
    for t, p in zip(y5_true, y5_pred):
        YT5 += t
        YP5 += p
    assert len(YT5) == len(YP5)
    ...
    p3, r3, f1s3, s3 = metrics.precision_recall_fscore_support(y_pred=YP3, y_true=YT3, average='macro', warn_for=tuple())
    print("label2:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p3, r3, f1s3))

    p4, r4, f1s4, s4 = metrics.precision_recall_fscore_support(y_pred=YP4, y_true=YT4, average='macro', warn_for=tuple())
    print("label2:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p4, r4, f1s4))

    p5, r5, f1s5, s5 = metrics.precision_recall_fscore_support(y_pred=YP5, y_true=YT5, average='macro', warn_for=tuple())
    print("label2:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p5, r5, f1s5))
...
def main():
    ...
    if args.train:
        ...
        # Model
        model = Token_indicator_multitask_se_1231(
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # backward pass
                loss_ote, loss_ts, ote2ts, ote2ts2, p_y_x_ts_tilde, p_y_x_ts_softmax, stance_loss, stance_loss2, argument_t_loss = splits[0], splits[1], splits[2], splits[3], splits[4], splits[5], splits[6], splits[7], splits[8]
                loss.backward()
                ...
                if step % 50 == 0:
                    ...
                    print(ote2ts)
                    print(ote2ts2)
                    print(p_y_x_ts_tilde)
                    print(p_y_x_ts_softmax)
                    print(stance_loss)
                    print(stance_loss2)
