...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    return y_true, y_pred, p, r, f1s,batch_input_ids
...
def main():
    ...
    #
    parser.add_argument('--num_labels', type=int, default=2, help='Number of labels; either 3 (pro, con, non) or 2 (arg, non).')
    #标签数量
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            y_true_train, y_pred_train, p_train, r_train, f1s_train,batch_input_ids_train = evaluation(
                    train_dataloader, model=model, device=device, tokenizer=tokenizer)
            print("TRAIN:  Pre. %.3f | Rec. %.3f | F1 %.3f"%(p_train, r_train, f1s_train))
            
            # EVALUATION: DEV SET
            y_true_dev, y_pred_dev, p_dev, r_dev, f1s_dev,batch_input_ids_dev = evaluation(
            ...
            if f1s_dev > best_f1s_dev:
                ...
                # EVALUATION: TEST SET
                y_true_test, y_pred_test, p_test, r_test, f1s_test,batch_input_ids_test = evaluation(
