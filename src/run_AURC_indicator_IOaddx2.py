...
from models2 import Token_indicator_multitask_addx2
...
#         pass
#
# sys.stdout = Logger("./3_1211x2.txt")
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        batch = tuple(t.to(device) for t in batch)
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_argument_input_ids, batch_argument_input_mask, batch_argument_sentence_ids,  batch_label_ids, batch_IO_label_ids, batch_stance_label_ids, batch_argument_label_ids = batch
        ...
        label_tagsx2_1 = tagssss[3]
        label_tagsx2_2 = tagssss[4]
        stance_tags1 = tagssss[5]
        stance_tags2 = tagssss[6]
        stance_tagsx2_1 = tagssss[7]
        stance_tagsx2_2 = tagssss[8]

        s1_pred+=stance_tags1.cpu().tolist()
        s2_pred+=stance_tags2.cpu().tolist()

        sx2_1_pred += stance_tagsx2_1.cpu().tolist()
        sx2_2_pred += stance_tagsx2_2.cpu().tolist()

        s1_true += batch_stance_label_ids.cpu().tolist()
        s2_true += batch_stance_label_ids.cpu().tolist()

        sx2_1_true += batch_stance_label_ids.cpu().tolist()
        sx2_2_true += batch_stance_label_ids.cpu().tolist()
        ...
        batch_correct_label_idsx2_1 = []
        batch_correct_tagsx2_1 = []
        batch_correct_label_idsx2_2 = []
        batch_correct_tagsx2_2 = []

        for input_ids, IO_label_ids, label_ids, IO_tag, tags1, tags2, tagsx2_1, tagsx2_2 in zip(batch_input_ids, batch_IO_label_ids, batch_label_ids, IO_tags, label_tags1, label_tags2, label_tagsx2_1, label_tagsx2_2):
            ...
            #第一层转移矩阵预测
            correct_label_ids2 = [l for t, l in zip(input_tokens[1:seq_len], label_ids[1:seq_len]) if not t.startswith('##')]
            correct_tags2 = [l for t, l in zip(input_tokens[1:seq_len], tags2[1:seq_len]) if not t.startswith('##')]
            seq = " ".join(input_tokens[1:seq_len]).replace(' ##', '')
            assert len(correct_label_ids2) == len(seq.split(' '))
            batch_correct_label_ids2.append(correct_label_ids2)  # 真实值
            batch_correct_tags2.append(correct_tags2)  # 预测值



            # 第二层label输出
            correct_label_idsx2_1 = [l for t, l in zip(input_tokens[1:seq_len], label_ids[1:seq_len]) if not t.startswith('##')]
            correct_tagsx2_1 = [l for t, l in zip(input_tokens[1:seq_len], tagsx2_1[1:seq_len]) if not t.startswith('##')]
            seq = " ".join(input_tokens[1:seq_len]).replace(' ##', '')
            assert len(correct_label_idsx2_1) == len(seq.split(' '))
            batch_correct_label_idsx2_1.append(correct_label_idsx2_1)  # 真实值
            batch_correct_tagsx2_1.append(correct_tagsx2_1)  # 预测值


            # 第一层转移矩阵预测
            correct_label_idsx2_2 = [l for t, l in zip(input_tokens[1:seq_len], label_ids[1:seq_len]) if not t.startswith('##')]
            # tags = [IO2ids[id2IO[l]] for l in tags]
            correct_tags2x2_2 = [l for t, l in zip(input_tokens[1:seq_len], tagsx2_2[1:seq_len]) if not t.startswith('##')]
            seq = " ".join(input_tokens[1:seq_len]).replace(' ##', '')
            assert len(correct_label_idsx2_2) == len(seq.split(' '))
            batch_correct_label_idsx2_2.append(correct_label_idsx2_2)  # 真实值
            batch_correct_tagsx2_2.append(correct_tags2x2_2)  # 预测值


        y1_true += batch_correct_label_ids1
        y1_pred += batch_correct_tags1
        ...
        yx2_1_true += batch_correct_label_idsx2_1
        yx2_1_pred += batch_correct_tagsx2_1

        yx2_2_true += batch_correct_label_idsx2_2
        yx2_2_pred += batch_correct_tagsx2_2
    # flatten
    YT, YP = [], []
    for t, p in zip(y1_true, y1_pred):
        YT += t
        YP += p
    ...
    YTx2_1, YPx2_1 = [], []
    for t, p in zip(yx2_1_true, yx2_1_pred):
        YTx2_1 += t
        YPx2_1 += p
    assert len(YT) == len(YP)

    YTx2_2, YPx2_2 = [], []
    for t, p in zip(yx2_2_true, yx2_2_pred):
        YTx2_2 += t
        YPx2_2 += p
    assert len(YTx2_2) == len(YPx2_2)
    ...
    #p_IO2, r_IO2, f1s_IO2, s_IO2 = metrics.precision_recall_fscore_support(y_pred=IO_YP, y_true=IO_YT, warn_for=tuple())
    ...
    px2_1, rx2_1, f1sx2_1, sx2_1 = metrics.precision_recall_fscore_support(y_pred=YPx2_1, y_true=YTx2_1, average='macro', warn_for=tuple())
    print("labelx2_1:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (px2_1, rx2_1, f1sx2_1))

    px2_2, rx2_2, f1sx2_2, sx2_2 = metrics.precision_recall_fscore_support(y_pred=YPx2_2, y_true=YTx2_2, average='macro',
                                                               warn_for=tuple())
    print("labelx2_2:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (px2_2, rx2_2, f1sx2_2))
    ...
    px2_1_s, rx2_1_s, f1sx2_1_s, sx2_1_s = metrics.precision_recall_fscore_support(y_pred=sx2_1_pred, y_true=sx2_1_true, average='macro',
                                                                   warn_for=tuple())
    print("stancex2_1:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (px2_1_s, rx2_1_s, f1sx2_1_s))

    px2_2_a, rx2_2_a, f1sx2_2_a, sx2_2_a = metrics.precision_recall_fscore_support(y_pred=sx2_2_pred, y_true=sx2_2_true, average='macro',
                                                                   warn_for=tuple())
    print("stancex2_2:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (px2_2_a, rx2_2_a, f1sx2_2_a))


    return y1_true, yx2_1_pred, yx2_2_pred, sx2_1_pred, sx2_2_pred, p, r, f1s
...
def main():
    ...
    # 数据位置
    parser.add_argument('--output_dir', type=str, default='../models_gpu3/',
    ...
    for topic, AD in AURC_DATA_dict.items():
        for ad in AD:
            ...
            sequence_argument_dict = tokenizer.encode_plus(ad['sentence'], topic, max_length=args.max_sequence_length+10,
                                                  pad_to_max_length=True, add_special_tokens=True)
            ...
            start_positions = [0] * (len(input_labels)-1)
            end_positions = [0] * (len(input_labels)-1)
            if ad['is_argument'] == 1:
                start_positions[ad['start']] = 1
    ...
    if args.train:
        ...
        # Model
        model = Token_indicator_multitask_addx2(
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                loss_ote, loss_ts1, loss_ts2, loss_tsx2_1, loss_tsx2_2, stance_loss1, stance_loss2, stance_lossx2_1, stance_lossx2_2, argument_t_loss= splits[0], splits[1], splits[2], splits[3], splits[4], splits[5], splits[6], splits[7], splits[8], splits[9]
                # backward pass
                ...
                if step % 50 == 0:
                    ...
                    print(stance_loss1)
                    print(stance_loss2)
                    print(stance_lossx2_1)
                    print(stance_lossx2_2)
