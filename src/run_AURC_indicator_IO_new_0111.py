...
from utils import InputFeatures_indicator_new, get_data_with_labels

from models2 import Token_indicator_multitask_se_0111
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        batch = tuple(t.to(device) for t in batch)
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_argument_input_ids, batch_argument_input_mask, batch_argument_sentence_ids, batch_label_ids, batch_pro_label_ids, batch_con_label_ids, batch_IO_label_ids, batch_pro_IO_label_ids, batch_con_IO_label_ids, batch_stance_label_ids, batch_pro_stance_label_ids, batch_con_stance_label_ids, batch_argument_label_ids = batch
        ...
        pro_IO_tags = tagssss[0]
        con_IO_tags = tagssss[1]
        label_tags = tagssss[2]
        stance_tags1 = tagssss[3]
        stance_tags2 = tagssss[4]
        s1_pred+=stance_tags1.cpu().tolist()
        s2_pred+=stance_tags2.cpu().tolist()
        s1_true += batch_pro_stance_label_ids.cpu().tolist()
        s2_true += batch_con_stance_label_ids.cpu().tolist()
        ...
        for input_ids, pro_IO_label_ids, con_IO_label_ids, label_ids, pro_IO_tag, con_IO_tag, tags in zip(batch_input_ids, batch_pro_IO_label_ids, batch_con_IO_label_ids, batch_label_ids, pro_IO_tags, con_IO_tags, label_tags):
            input_ids = input_ids.cpu().tolist()
            pro_IO_label_ids = pro_IO_label_ids.cpu().tolist()
            con_IO_label_ids = con_IO_label_ids.cpu().tolist()
            label_ids = label_ids.cpu().tolist()

            if type(tags) != list:
                tags = tags.cpu().tolist()
                pro_IO_tag = pro_IO_tag.cpu().tolist()
                con_IO_tag = con_IO_tag.cpu().tolist()
            ...
            correct_pro_IO_ids = [l for t, l in zip(input_tokens[1:seq_len], pro_IO_label_ids[1:seq_len]) if not t.startswith('##')]
            pre_pro_IO = [l for t, l in zip(input_tokens[1:seq_len], pro_IO_tag[1:seq_len]) if not t.startswith('##')]
            batch_pro_IO_label.append(correct_pro_IO_ids)  # 真实值
            batch_pro_IO_tags.append(pre_pro_IO)  # 预测值

            correct_con_IO_ids = [l for t, l in zip(input_tokens[1:seq_len], con_IO_label_ids[1:seq_len]) if not t.startswith('##')]
            pre_con_IO = [l for t, l in zip(input_tokens[1:seq_len], con_IO_tag[1:seq_len]) if not t.startswith('##')]
            batch_con_IO_label.append(correct_con_IO_ids)  # 真实值
            batch_con_IO_tags.append(pre_con_IO)  # 预测值
        ...
        y_pro_IO_true += batch_pro_IO_label
        y_pro_IO_pred += batch_pro_IO_tags

        y_con_IO_true += batch_con_IO_label
        y_con_IO_pred += batch_con_IO_tags
    # flatten
    YT, YP = [], []
    ...
    pro_IO_YT, pro_IO_YP = [], []
    for t, p in zip(y_pro_IO_true, y_pro_IO_pred):
        pro_IO_YT += t
        pro_IO_YP += p
    assert len(pro_IO_YT) == len(pro_IO_YP)

    con_IO_YT, con_IO_YP = [], []
    for t, p in zip(y_con_IO_true, y_con_IO_pred):
        con_IO_YT += t
        con_IO_YP += p
    assert len(con_IO_YT) == len(con_IO_YP)

    p_IO, r_IO, f1s_IO, s_IO = metrics.precision_recall_fscore_support(y_pred=pro_IO_YP, y_true=pro_IO_YT, average='macro', warn_for=tuple())
    print("pro_IO:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p_IO, r_IO, f1s_IO))

    p2, r2, f1s2, s2 = metrics.precision_recall_fscore_support(y_pred=con_IO_YP, y_true=con_IO_YT, average='macro',warn_for=tuple())
    print("con_IO:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p2, r2, f1s2))

    p, r, f1s, s = metrics.precision_recall_fscore_support(y_pred=YP, y_true=YT, average='macro', warn_for=tuple())
    print("label:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p, r, f1s))



    p_s, r_s, f1s_s, s_s = metrics.precision_recall_fscore_support(y_pred=s1_pred, y_true=s1_true, average='macro', warn_for=tuple())
    print("stance_pro:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p_s, r_s, f1s_s))

    p_a, r_a, f1s_a, s_a = metrics.precision_recall_fscore_support(y_pred=s2_pred, y_true=s2_true, average='macro', warn_for=tuple())
    print("stance_con:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p_a, r_a, f1s_a))



    return y_true, y_pred, y_pred, s1_pred, s1_pred, p, r, f1s
...
def main():
    ...
    # 数据位置
    parser.add_argument('--output_dir', type=str, default='../models_gpu2/',
    ...
    stance_label2id['pro'] = 1
    stance_label2id['con'] = 2

    argument_label2id = dict()
    argument_label2id['non'] = 0
    argument_label2id['pro'] = 1
    argument_label2id['con'] = 1
    ...
    for topic, AD in AURC_DATA_dict.items():
        for ad in AD:
            ...
            for i in range(len(pro_label)):
                add_label.append(pro_label[i]+con_label[i])
            #a=pro_label+con_label
            assert input_labels==add_label
            sequence_dict['pro_label_ids'] = pro_label
            sequence_dict['con_label_ids'] = con_label

            input_IO_labels = [IO_label2id[label] for label in ad['tokenized_sentence_bert_labels'].split(' ')]
            input_IO_labels = [0] + input_IO_labels[:args.max_sequence_length - 1] + [0] * max(0, args.max_sequence_length - len(input_IO_labels) - 1)
            sequence_dict['IO_ids'] = input_IO_labels

            pro_IO_labels = [label for label in pro_label]
            #pro_IO_labels = [0] + pro_IO_labels[:args.max_sequence_length - 1] + [0] * max(0, args.max_sequence_length - len(pro_IO_labels) - 1)
            sequence_dict['pro_IO_ids'] = pro_IO_labels

            con_IO_labels = [con_IO_label2id[label] for label in con_label]
            #con_IO_labels = [0] + con_IO_labels[:args.max_sequence_length - 1] + [0] * max(0, args.max_sequence_length - len(con_IO_labels) - 1)
            sequence_dict['con_IO_ids'] = con_IO_labels

            add_IO_label=[]
            for i in range(len(input_IO_labels)):
                add_IO_label.append(pro_IO_labels[i]+con_IO_labels[i])
            #a=pro_label+con_label
            assert input_IO_labels==add_IO_label
            ...
            if ad['sentence_level_stance'] == "pro":
                # sequence_dict['pro_stance_ids'] = [0, 1]
                # sequence_dict['con_stance_ids'] = [1, 0]
                sequence_dict['pro_stance_ids'] = 1
                sequence_dict['con_stance_ids'] = 0
            elif ad['sentence_level_stance'] == "non":
                # sequence_dict['pro_stance_ids'] = [1, 0]
                # sequence_dict['con_stance_ids'] = [1, 0]
                sequence_dict['pro_stance_ids'] = 0
                sequence_dict['con_stance_ids'] = 0
            elif ad['sentence_level_stance'] == "con":
                # sequence_dict['pro_stance_ids'] = [1, 0]
                # sequence_dict['con_stance_ids'] = [0, 1]
                sequence_dict['pro_stance_ids'] = 0
                sequence_dict['con_stance_ids'] = 1
            sequence_dict['stance_ids'] = stance_label2id[ad['sentence_level_stance']]
            sequence_dict['argument_ids'] = argument_label2id[ad['sentence_level_stance']]
            ...
            FE = [
                InputFeatures_indicator_new(
                    ...
                    pro_label_ids = sequence_dict['pro_label_ids'],
                    con_label_ids = sequence_dict['con_label_ids'],
                    IO_label_ids = sequence_dict['IO_ids'],
                    pro_IO_ids = sequence_dict['pro_IO_ids'],
                    con_IO_ids = sequence_dict['con_IO_ids'],
                    stance_label = sequence_dict['stance_ids'],
                    pro_stance_ids = sequence_dict['pro_stance_ids'],
                    con_stance_ids = sequence_dict['con_stance_ids'],
    ...
    all_pro_label_ids = torch.tensor([f.pro_label_ids for f in train_features], dtype=torch.long)
    all_con_label_ids = torch.tensor([f.con_label_ids for f in train_features], dtype=torch.long)
    all_IO_label_ids = torch.tensor([f.IO_label_ids for f in train_features], dtype=torch.long)
    all_pro_IO_label_ids = torch.tensor([f.pro_IO_ids for f in train_features], dtype=torch.long)
    all_con_IO_label_ids = torch.tensor([f.con_IO_ids for f in train_features], dtype=torch.long)
    all_stance_label_ids = torch.tensor([f.stance_label for f in train_features], dtype=torch.long)
    all_pro_stance_label_ids = torch.tensor([f.pro_stance_ids for f in train_features], dtype=torch.long)
    all_con_stance_label_ids = torch.tensor([f.con_stance_ids for f in train_features], dtype=torch.long)
    all_argument_label_ids = torch.tensor([f.argument_label for f in train_features], dtype=torch.long)
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_pro_label_ids, all_con_label_ids, all_IO_label_ids, all_pro_IO_label_ids, all_con_IO_label_ids, all_stance_label_ids, all_pro_stance_label_ids, all_con_stance_label_ids, all_argument_label_ids)
    ...
    all_pro_label_ids = torch.tensor([f.pro_label_ids for f in eval_features], dtype=torch.long)
    all_con_label_ids = torch.tensor([f.con_label_ids for f in eval_features], dtype=torch.long)
    all_IO_label_ids = torch.tensor([f.IO_label_ids for f in eval_features], dtype=torch.long)
    all_pro_IO_label_ids = torch.tensor([f.pro_IO_ids for f in eval_features], dtype=torch.long)
    all_con_IO_label_ids = torch.tensor([f.con_IO_ids for f in eval_features], dtype=torch.long)
    all_stance_label_ids = torch.tensor([f.stance_label for f in eval_features], dtype=torch.long)
    all_pro_stance_label_ids = torch.tensor([f.pro_stance_ids for f in eval_features], dtype=torch.long)
    all_con_stance_label_ids = torch.tensor([f.con_stance_ids for f in eval_features], dtype=torch.long)
    all_argument_label_ids = torch.tensor([f.argument_label for f in eval_features], dtype=torch.long)
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_pro_label_ids, all_con_label_ids, all_IO_label_ids, all_pro_IO_label_ids, all_con_IO_label_ids, all_stance_label_ids, all_pro_stance_label_ids, all_con_stance_label_ids, all_argument_label_ids)
    ...
    all_pro_label_ids = torch.tensor([f.pro_label_ids for f in test_features], dtype=torch.long)
    all_con_label_ids = torch.tensor([f.con_label_ids for f in test_features], dtype=torch.long)
    all_IO_label_ids = torch.tensor([f.IO_label_ids for f in test_features], dtype=torch.long)
    all_pro_IO_label_ids = torch.tensor([f.pro_IO_ids for f in test_features], dtype=torch.long)
    all_con_IO_label_ids = torch.tensor([f.con_IO_ids for f in test_features], dtype=torch.long)
    all_stance_label_ids = torch.tensor([f.stance_label for f in test_features], dtype=torch.long)
    all_pro_stance_label_ids = torch.tensor([f.pro_stance_ids for f in test_features], dtype=torch.long)
    all_con_stance_label_ids = torch.tensor([f.con_stance_ids for f in test_features], dtype=torch.long)
    all_argument_label_ids = torch.tensor([f.argument_label for f in test_features], dtype=torch.long)
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_pro_label_ids, all_con_label_ids, all_IO_label_ids, all_pro_IO_label_ids, all_con_IO_label_ids, all_stance_label_ids, all_pro_stance_label_ids, all_con_stance_label_ids, all_argument_label_ids)
    ...
    if args.train:
        ...
        # Model
        model = Token_indicator_multitask_se_0111(
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                batch = tuple(t.to(device) for t in batch)
                batch_input_ids, batch_input_mask, batch_sentence_ids, batch_argument_input_ids, batch_argument_input_mask, batch_argument_sentence_ids, batch_label_ids, batch_pro_label_ids, batch_con_label_ids, batch_IO_label_ids, batch_pro_IO_label_ids, batch_con_IO_label_ids, batch_stance_label_ids, batch_pro_stance_label_ids, batch_con_stance_label_ids, batch_argument_label_ids = batch
                #all_input_ids, all_input_mask, all_segment_ids, all_argument_input_ids, all_argument_input_mask, all_argument_segment_ids, all_label_ids, all_pro_label_ids, all_con_label_ids, all_IO_label_ids, all_pro_IO_label_ids, all_con_IO_label_ids, all_stance_label_ids, all_pro_stance_label_ids, all_con_stance_label_ids, all_argument_label_ids
                # print(batch_input_ids.shape[0])
                # forward pass
                loss, splits = model(
                    batch_input_ids,
                    ...
                    argument_attention_mask=batch_argument_input_mask,
                    labels=batch_label_ids,
                    pro_labels=batch_pro_label_ids,
                    con_labels=batch_con_label_ids,
                    IO_labels=batch_IO_label_ids,
                    pro_IO_labels=batch_pro_IO_label_ids,
                    con_IO_labels=batch_con_IO_label_ids,
                    stance_label=batch_stance_label_ids,
                    pro_stance_label=batch_pro_stance_label_ids,
                    con_stance_label=batch_con_stance_label_ids,
                    argument_label=batch_argument_label_ids,
                    # batch_size = batch_input_ids.shape[0]
                )

                loss_ote_pro, loss_ote_con, loss_ote, loss_ts, loss_ts_pro, loss_ts_con, pro_stance_loss, con_stance_loss= splits[0], splits[1], splits[2], splits[3], splits[4], splits[5], splits[6], splits[7]
                # backward pass
                loss.backward()
                ...
                if step % 50 == 0:
                    ...
                    print(loss_ts_pro)
                    print(loss_ts_con)
                    print(pro_stance_loss)
                    print(con_stance_loss)
                    print('-----------------------------------------')
