...
#from models2 import Token_pipeline_gru_crf_step
#from models2 import Token_pipeline_idcnn_crf_step
from models2 import Token_pipeline_lstm_crf_step#Li
...
sys.stdout = Logger("./1_pipeline_step2.txt")
...
def evaluation(sample_dataloader, model, device, tokenizer):
        ...
        with torch.no_grad():
            for i in batch_label_ids.cpu().tolist():
                ...
                for k in range(len(starts)):
                    ...
                    stance_pres.append(stance_pre.cpu().tolist()[0])
                    stance_true.append(label_stance[k])
                    assert len(stance_true) == len(stance_pres)
    ...
    p_stance, r_stance, f1s_stance, s_stance = metrics.precision_recall_fscore_support(y_pred=stance_pres,
                                                                                       y_true=stance_true,
                                                                                       average='macro',
                                                                                       warn_for=tuple())
    print("stance:  Pre. %.3f | Rec. %.3f | F1 %.3f" % (p_stance, r_stance, f1s_stance))

    return p_stance, r_stance, f1s_stance
...
def main():
    ...
    if args.train:
        ...
        # Model5
        model = Token_pipeline_lstm_crf_step(
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # forward pass
                if batch_argument_label_ids.cpu().tolist()[0]==1:
                    for i in batch_label_ids.cpu().tolist():
                        starts = []
                        ends = []
                        label_stance = []
                ...
                if step % 200 == 0:
                    ...
                    if f1s_dev >= best_f1s_dev:
                        ...
                        # EVALUATION: TEST SET
                        p_test, r_test, f1s_test = evaluation(
