...
from models2 import Token_indicator_multitask_new_0117
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        for i in stance_tags1.cpu().tolist():
            count=0
            counts=[0,0,0]
            for j in i:
                if j>0.6:
                    counts[count]=1
                count+=1
            s1_pred.append(counts)
        #from sklearn.preprocessing import MultiLabelBinarizer
        #s1_pred = MultiLabelBinarizer().fit_transform(s1_preds).tolist()
        #s1_pred+=stance_tags1.cpu().tolist()
        #s2_pred+=stance_tags2.cpu().tolist()
        for i in stance_tags2.cpu().tolist():
            count=0
            counts=[0,0,0]
            for j in i:
                if j>0.6:
                    counts[count]=1
                count+=1
            s2_pred.append(counts)
        #s2_pred = MultiLabelBinarizer().fit_transform(s2_preds).tolist()
...
def main():
    ...
    if args.train:
        ...
        # Model
        model = Token_indicator_multitask_new_0117(
